...
def _run_simulation(dem_path, fdir_path, output_dir, cn, moisture,
    ...
    for i in range(n_steps):
        ...
        if earthwork_stores:
            # Distribute runoff proportionally to earthwork catchment areas
            total_store_area = sum(s.area_m2 for s in earthwork_stores)
            catchment_m2 = shape[0] * shape[1] * cell_area_m2
            for store in earthwork_stores:
                fraction = store.area_m2 / catchment_m2 if catchment_m2 > 0 else 0.0
                store.inflow_m3 += total_runoff_m3_step * fraction
