...
class TerrainFlowAssessment:
    ...
    def _apply_rank_style(self, layer, max_acc=None):
        """Single-symbol renderer with data-defined blue ramp by peak_acc.
        Produces a flat (non-grouped) layer entry so click tools see all features."""
        from qgis.core import QgsLineSymbol
        if max_acc is None or max_acc == 0:
            max_acc = 1.0
        # #aed6f1 (174,214,241) → #1a5276 (26,82,118)
        color_expr = (
            f'color_rgb('
            f'scale_linear("peak_acc",0,{max_acc},174,26),'
            f'scale_linear("peak_acc",0,{max_acc},214,82),'
            f'scale_linear("peak_acc",0,{max_acc},241,118)'
            f')'
        )
        symbol = QgsLineSymbol.createSimple({"width": "0.8", "capstyle": "round"})
    ...
    def _display_swale_segments(self, segments):
        ...
        # Data-defined green ramp by inflow_m3 (flat layer, no sub-groups)
        # #a9dfbf (169,223,191) → #1e8449 (30,132,73)
        from qgis.core import QgsLineSymbol
        max_inflow = max((s.inflow_m3 or 0) for s in segments) if segments else 1.0
        if max_inflow == 0:
            max_inflow = 1.0
        color_expr = (
            f'color_rgb('
            f'scale_linear("inflow_m3",0,{max_inflow},169,30),'
            f'scale_linear("inflow_m3",0,{max_inflow},223,132),'
            f'scale_linear("inflow_m3",0,{max_inflow},191,73)'
            f')'
        )
    ...
    def _generate_simple_contours(self):
        ...
        try:
            import processing
            interval = self.panel.contour_interval_m
    ...
    def _ensure_ew_layers(self):
        ...
        for ew_type, (geom_type, display_name, color_hex, fill_hex, width) in self._EW_STYLES.items():
            ...
            if geom_type == "LineString":
                ...
                # Polygon — semi-transparent fill, solid outline
                ...
                sym.symbolLayer(0).setColor(fill_color)
