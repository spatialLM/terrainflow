...
SPILLWAY_RIM = (
    "The lowest point of natural ground around this feature — where water would escape\n"
    "if you dug the hole and built nothing else. Sampled from the DEM on the ring of\n"
    "cells just outside the footprint.\n\n"
    "It is not the top lip of the cut, and not where you started drawing: for a contour\n"
    "swale the lowest surrounding ground is usually at one of the ends. For a dam it is\n"
    "the wall crest instead, because there the wall is the containment.\n\n"
    "This is the datum the crest is measured against, because it is the elevation the\n"
    "terrain actually supplies. A crest typed without reference to it is a number with\n"
    "nothing behind it.\n\n"
    "A companion berm is not included. The berm is built on the downhill side only, so\n"
    "where the low point is at an end it raises nothing — counting it would claim\n"
    "headroom the ground may not have. The figure is therefore conservative for a\n"
    "bermed swale: the real spill level is at or above this."
)
SPILLWAY_CREST = (
    "Absolute elevation of the spillway crest — the level water reaches before it\n"
    "starts leaving. This is the setting-out level: the height you cut the sill to on\n"
    "the ground.\n\n"
    "Note the reported storage is still measured to the full depth of the feature, not\n"
    "down to this line, so a crest set well below the rim holds less than the capacity\n"
    "figure claims. What the crest does govern here is the freeboard budget — the gap\n"
    "between it and the rim is what has to accommodate the design head and still leave\n"
    "a margin.\n\n"
