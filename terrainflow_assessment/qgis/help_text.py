...
SPILLWAY_CREST = (
    "Absolute elevation of the spillway sill — the level water reaches before it\n"
    "starts leaving. This is the authoritative value: it is what would be set out on\n"
    "the ground and what gets cut into the terrain model.\n\n"
    "Read-only, because it is a consequence rather than a decision: the containment\n"
    "level less the spillway depth you set above. On a swale you may set it out the\n"
    "other way instead, from the trench floor — see 'Height above floor'.\n\n"
    "Note the reported storage is still measured to the full depth of the feature, not\n"
    "down to this line, so a sill set well below the spill level holds less than the\n"
    "capacity figure claims — which is what the storage row underneath says in cubic\n"
    "metres."
)
SPILLWAY_HEIGHT_ABOVE_FLOOR = (
    "How far the sill stands above the floor of the feature — the same sill as the\n"
    "two rows above, measured up from the bottom rather than down from the top.\n\n"
    "Editable, and the one exception to spillway depth and built width being the only\n"
    "figures you set. Set out from whichever end suits the job: type a depth and this\n"
    "follows, or type this and the depth follows. They cannot disagree.\n\n"
    "This is usually the row to set out from: it is the one measurement a builder can\n"
    "take with a staff standing in the excavation, and it does not depend on ground\n"
    "somewhere else. The other datum is a single lowest point over the whole footprint,\n"
    "and on a contour swale that point is usually at one of the ends — so a notch cut\n"
    "half way along a falling swale and measured down from an end is measured from\n"
    "ground a hundred metres away, at an elevation it does not share.\n\n"
    "On a basin the rim does not fall along a run the way a swale's does, so the depth\n"
    "below it is normally the easier figure to think in. The row is offered all the\n"
    "same: a basin is a cut feature with a floor, and setting out from the floor is a\n"
    "fact about having one, not about the shape of the rim.\n\n"
...
SPILLWAY_DEPTH = (
    "How deep the spillway is cut below the level this feature is held to. This is the\n"
    "one dimension of the overflow you set; everything under it follows from it.\n\n"
    "The datum is the containment level, not necessarily natural ground: on a swale\n"
    "with a companion berm it is the berm crest, so this is the depth of a notch in\n"
    "what was built. Where the two differ the dialog shows both.\n\n"
    "It buys two things at once, and they pull against each other. Depth is what the\n"
    "design flow runs in — too little and the sill cannot pass the storm — and it is\n"
    "also the freeboard, the clear margin that keeps the water off the surrounding\n"
    "ground so the overflow happens where you armoured it. The Freeboard row below\n"
    "shows what is left once the flow has taken its share.\n\n"
    "It costs storage: every centimetre of sill is a centimetre the feature no longer\n"
    "holds, which the storage row states in cubic metres as you move it.\n\n"
    "0.00 m is allowed. It means a sill level with the containing ground — no notch at\n"
    "all, the bank simply overtopping at its lowest point. That is a thing you may need\n"
...
SPILLWAY_FREEBOARD = (
    "Clear height left between the design water surface and the lowest containing\n"
    "ground: the spillway depth, less the depth the design flow runs at.\n\n"
    "A readout, not a setting. It is the reason not to cut the sill shallow, and the\n"
    "reason a deeper sill is worth its lost storage — so it is shown where the trade\n"
    "is being made rather than asked for separately.\n\n"
    "This is the margin that makes the spillway the actual control rather than merely\n"
    "the intended one. At zero the water surface at design flow reaches the\n"
    "surrounding rim, water escapes there too, and the choice of where to overflow is\n"
    "given up. It is drawn red there, and amber below what this type is designed for.\n\n"
    "What the type asks for depends on what you are building, because the published\n"
    "figure is an embankment one. NRCS Conservation Practice Standard 378 requires\n"
    "0.30 m between the design flow level and the top of a settled dam wall — a wall\n"
    "that would be breached if overtopped. A cut swale has no wall: its containment is\n"
    "undisturbed ground, and the failure is water leaving at an unarmoured point rather\n"
    "than a structure giving way. Holding a swale to the dam figure spends more depth\n"
    "than a shallow swale has.\n\n"
    "A design under that figure is still reported, and still called out — it is a real\n"
    "reduction in margin and not a formality."
)
SPILLWAY_INLET = (
