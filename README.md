...
| Output | Description |
|--------|-------------|
| Flow accumulation raster | Maps where surface water concentrates across the landscape |
| Drainage network | Extracts the surface drainage network from flow accumulation thresholds |
| Catchment boundary | Delineates the contributing area draining to your site |
| Exit points | Identifies where water leaves the site boundary |
...
| Type | Description |
|------|-------------|
| Swale | Cut channel along contour; optional volume-conserved companion berm placed on downhill side |
| Berm | Raised barrier; deflects or slows surface flow |
| Basin | Polygon detention area; water ponds within the excavated footprint |
| Dam | Wall set to absolute crest elevation; water pools behind up to the crest |
| Diversion drain | Graded channel designed to intercept and redirect flow; gradient and discharge calculated via Manning's equation |
...
1. Open QGIS and enable the TerrainFlow panel (*Plugins → TerrainFlow → Toggle Panel*)
2. Load a DEM from file or select an existing raster layer
3. Optionally set a site boundary polygon to clip analysis to your area of interest
4. Click **Run Baseline Analysis** — TerrainFlow generates flow accumulation, drainage network, and catchment boundary as QGIS layers
5. Draw earthworks using the panel tools (swale, berm, basin, dam, diversion drain)
6. Click **Run with Earthworks** — TerrainFlow burns the earthworks into the DEM and re-runs the analysis
7. Compare baseline and post-earthworks layers side by side; use the Ponding Query Tool to inspect ponding volumes
...
- **LINZ LiDAR data** — [data.linz.govt.nz](https://data.linz.govt.nz) (high-resolution LiDAR DEMs for most of NZ)
...
- [x] DEM loading and preprocessing (pit filling, D-infinity and D8 routing)
- [x] Flow direction, flow accumulation, drainage network extraction
- [x] Catchment contributing area preview
- [x] Site boundary support and exit point detection
- [x] Full QGIS docked panel GUI
- [x] Interactive earthwork drawing tools (line, polygon, point)
- [x] DEM burning — swale, berm, basin, dam, diversion drain
- [x] Volume-conserved companion berm placement
- [x] Manning's equation discharge for diversion drains
- [x] SCS Curve Number rainfall-runoff with AMC adjustments
